...
FixENM_Morse::FixENM_Morse(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
  int i,j,tmp;
  bool not_find;
  ...
  if (me == 0) printf("Now fix ENM_Morse!\n");  
  if (me == 0 && logfile) fprintf(logfile,"Now fix ENM_Morse!\n");
  //1. Check Args
  if ( narg != 7)
    error->all(FLERR,"Invalid ENM_Morse command!\nToo few args!\nUsage: fix fix_id group_id enm_morse v0_type_file alpha_type_file r0_type_file enm_bond_file");
  
  //2, Read v0_styles
  //2.1 Open the file
  if (me ==0) printf("ENM_Morse now reading v0 style file.\n");
  if (me == 0 && logfile) fprintf(logfile,"ENM_Morse now reading V0 style file.\n");
  fq = fopen(arg[3],"r");
  if (fq == NULL)   error->all(FLERR,"Cannot open V0 style file\n");
  //2.2 Read the number of v0 type
  fscanf(fq,"%d\n",&v0_type_num);
  //2.3 Read k type )
  memory->create(v0,v0_type_num+1,"fix:enm_morse_v0");
  //k = new double[k_type_num+1];//first is null
  for(i=1;i<=v0_type_num;i++) fscanf(fq,"%d,%lf\n",&tmp,&v0[i]);
  //2.4 Close the file 
  fclose(fq);
  ...
  //3.1 Open the file
  if (me ==0) printf("ENM_Morse now reading v0 style file.\n");
  if (me == 0 && logfile) fprintf(logfile,"ENM_Morse now reading alpha style file.\n");
  fq = fopen(arg[4],"r");
  if (fq == NULL)   error->all(FLERR,"Cannot open alpha style file\n");
  //3.2 Read the number of v0 type
  fscanf(fq,"%d\n",&alpha_type_num);
  //3.3 Read k type )
  memory->create(alpha,alpha_type_num+1,"fix:enm_morse_alpha");
  for(i=1;i<=alpha_type_num;i++) fscanf(fq,"%d,%lf\n",&tmp,&alpha[i]);
  //3.4 Close the file 
  fclose(fq);
   ...
   //4.2 Read the number of r0 type
   fscanf(fq,"%d\n",&r0_type_num);
   //4.3 Read r0 type
  ...
  //5.2 Read the number of bonds and get atom_num
  fscanf(fq,"%d\n",&bond_num);  
  //5.3 Read bond data
  ...
  for(i=0;i<bond_num;i++){
    //5.3.1 Read
    fscanf(fq,"%d,%d,%d,%d,%d,%lf,%d\n",&bond_list[i].tag1,&bond_list[i].tag2,&bond_list[i].v0_type,&bond_list[i].alpha_type,&bond_list[i].r0_type,&bond_list[i].fragment,&bond_list[i].ca_id);
    //5.3.2 Checking
    if (bond_list[i].v0_type <= 0 || bond_list[i].v0_type > v0_type_num || bond_list[i].alpha_type <= 0 || bond_list[i].alpha_type > alpha_type_num || bond_list[i].r0_type <= 0 || bond_list[i].r0_type > r0_type_num){
      sprintf(str,"Error found in line %d! Invalid bond type!\nV0 type, alpha type and r0 type should belong to [1,type_num]\n",i+1);
      error->all(FLERR,str);
    }
...
if (me==0) printf("Fix ENM Summary:\nENM V0 Type Number: %d\nENM r0 Type Number: %d\nENM Bond Number: %d\nPartial Bond Number: %d\n",v0_type_num,r0_type_num,bond_num,partial_bond_num);
if (me==0 && logfile) fprintf(logfile,"Fix ENM Summary:\nENM V0 Type Number: %d\nENM r0 Type Number: %d\nENM Bond Number: %d\nPartial Bond Number: %d\n",v0_type_num,r0_type_num,bond_num,partial_bond_num);
delete [] partial_bond;
//8, Initialize local_index
memory->create(local_index,natoms+1,"fix:enm_morse_local_index");
}


/* ---------------------------------------------------------------------- */

int FixENM_Morse::setmask()
{
  int mask = 0;
  mask |= POST_FORCE;
  mask |= THERMO_ENERGY;
...
double FixENM_Morse::compute_scalar(){
   double all;
...
void FixENM_Morse::post_force(int vflag)
{
    ...
    if (i1_in_local || i2_in_local){
      ...
      if (bond_list[n].fragment < 1.0){
        ...
        if (saved_partial_bond_data[bond_list[n].saved_id].calculated){
          ...
          ralpha = exp(-alpha[bond_list[n].alpha_type]*dr);
          if (r > 0.0) fbond = -2.0*v0[bond_list[n].v0_type]*alpha[bond_list[n].alpha_type]*(1-ralpha)*ralpha/r;
          else fbond = 0.0;
          ebond = v0[bond_list[n].v0_type]*(1-ralpha)*(1-ralpha);
          f_x = delx*fbond;
          f_y = dely*fbond;
          f_z = delz*fbond;
          v[0] = delx*delx*fbond;
